...
ethnicity <- read_excel("2018_JD_Enrollment_and_Ethnicity-2.xlsx") %>% 
  ...
  mutate(school_name = str_to_title(school_name)) %>% 
  mutate('Minority' = (minority_degrees_awd / total_degrees_awarded)*100) %>% 
...
ethnicity_test <- read_excel("2018_JD_Enrollment_and_Ethnicity-2.xlsx") %>% 
  ...
  gather(Minority, White, 'Unknown Race', International, key = "label", value = "percent") 

ethn_vis <- left_join(first_year, ethnicity_test, by = "school_name")
